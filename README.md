# ajazz-driver-reverse

Реверс-код HID-драйвера Ajazz: цель — **владеть протоколом клавиатур Ajazz**
...
> **Связь с другими репозиториями.** `models.json` (и слой протокола) —
> канонический источник для **`ajazz-tauri`**: его CI пинит конкретный коммит
> этого репо (файл `.reverse-rev`) и копирует `models.json`. UI-снапшот
> (`./app/`) сюда не коммитится; модели/протокол обновляются авто-PR-ами из
> пайплайна **`ajazz-driver-artifact`**.
